    ...
    if ($_POST) {
        try {
            ...
            if ($conn->connect_error) {
                ...
                if (!isset($_FILES["service_image"]) && isset($_POST["service_title"])) { // changing service name
                ...
                } else if (isset($_FILES["service_image"]) && isset($_POST["service_title"])) { //changing both category name and image.
                ...
                } else if (isset($_FILES["service_image"]) && !isset($_POST["service_title"])){ //changing category name
                    $temp = explode(".", $_FILES["cat_file"]["name"]); //getting current filename
                    $newfilename = round(microtime(true)) . '.' . end($temp); //setting new filename
                    move_uploaded_file($_FILES['cat_file']['tmp_name'],$location.$newfilename); //moving file to the server.
