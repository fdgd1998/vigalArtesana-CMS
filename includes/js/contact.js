...
$(document).ready(function(){
    ...
    $("#submit-message").on("click", function(){
        ShowSpinnerOverlay("Enviando mensaje...");
        if(grecaptcha.getResponse() == "") {
            alert("Ha ocurrido un fallo en la verificación de Google ReCaptcha. Inténtalo de nuevo un poco más tarde.");
        } else {
            var motivo = "";
            ...
            var message = "\
                ...
                <li>Nombre: <strong>"+$.trim($("#nombre").val())+"</strong></li>\
            ...
            formData.append("name", $.trim($("#nombre").val()));
            formData.append("subject", "Formulario de contacto - ViGal Artesana.");
            formData.append("body", message);
            formData.append("g-recaptcha-response", grecaptcha.getResponse());
            console.log(grecaptcha.getResponse());
            $.ajax({
                url: location.origin+'/scripts/check_recaptcha.php', // this is the target
                type: 'post', // method
                cache: false,
                ...
                success: function(response) { // if the http response code is 200
                    alert(response);
                    window.location.href = location.origin+"/contacto.php";
                },
